...
setupEnvironment()
{
   # put two lines of shell code here (but NOT the setting of any environment variables!)
   # (Hint: building a jar is the second line)
}
...
runProgram()
{
   java -ea -jar clever.jar
}

checkAndCompileProgram()
{
   if [ BegatOMatic5.java -nt BegatOMatic5.class ]
   then
      javac BegatOMatic5.java || exit 1
   fi
}
